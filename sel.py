...
def get_driver(headless: bool = True):
    try:
        ...
        # 🚀 Use webdriver-manager instead of hardcoded path
...
def display_stock_chart(ticker: str, timeframe: str = "12M", headless: bool = False):
    try:
        ...
        try:
            ...
            logger.info("Chart toolbar loaded")
            time.sleep(1)  # Additional wait for JavaScript
        except Exception as e:
            logger.warning(f"Chart toolbar not found, but proceeding: {e}")
        ...
        try:
            ...
            logger.info(f"Timeframe button found: {time_button.get_attribute('outerHTML')}")
            try:
                time_button.click()  # Try native click
                logger.info(f"Clicked {timeframe} button using native click")
            except:
                logger.info(f"Native click failed, trying JavaScript click")
                driver.execute_script("arguments[0].click();", time_button)
                logger.info(f"Clicked {timeframe} button using JavaScript")

            # Verify timeframe change by checking active state
            wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, f"//button[@data-name='{tf}' and contains(@class, 'isActive')]")
                )
            )
            logger.info(f"✅ Chart updated to {timeframe} view")
            driver.save_screenshot(f"chart_{ticker}_{timeframe}.png")
            logger.info(f"Screenshot saved for {ticker} at {timeframe}")

            # Log browser console for debugging
            console_logs = driver.get_log("browser")
            for log in console_logs:
                logger.info(f"Browser console: {log}")
